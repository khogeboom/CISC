...
int main()
{
	...
	else
	{
		cout << "Invalid password. Please enter a password "
			 << "with exactly 6 letters and 4 digits and "
             << "all lower case letters." << endl;
		cout << "For example, my37code99 is valid" << endl;
	}
...
bool testPassWord(char custPass[])
{
    ...
    {
        if (isalpha(custPass[i]))
            if (islower(custPass[i]) == false)
                isLowerCase = false;
    }

	if (numLetters == 6 && numDigits == 4 && length == 10 && isLowerCase == true)
		return true;
	else
		return false;
}
